...
abstract class AbstractBddStepDefinition {
    ...
    protected static final HeaderAndJson scenarioScope = new HeaderAndJson();
    ...
    void addQueryParameters(Map<String, String> newParams) {
        AssertionsForInterfaceTypes.assertThat(newParams).isNotEmpty();
    ...
    void addHeaders(Map<String, String> newHeaders) {
        AssertionsForInterfaceTypes.assertThat(newHeaders).isNotEmpty();
    ...
    void request(String resource, HttpMethod method) {
        ...
        AssertionsForInterfaceTypes.assertThat(method).isNotNull();
    ...
    List<String> checkHeaderExists(String headerName, boolean isNot) {
        ...
        AssertionsForInterfaceTypes.assertThat(responseEntity.getHeaders()).isNotNull();

        if (!isNot) {
            AssertionsForInterfaceTypes.assertThat(responseEntity.getHeaders().get(headerName)).isNotNull();
            return responseEntity.getHeaders().get(headerName);
        } else {
            AssertionsForInterfaceTypes.assertThat(responseEntity.getHeaders().get(headerName)).isNull();
            return null;
        }
    }
    ...
    private void checkJsonCollection(Collection currentValue, String jsonValue, boolean isNot) {
        AssertionsForInterfaceTypes.assertThat(currentValue).isNotEmpty();

        if (!isNot) {
            AssertionsForInterfaceTypes.assertThat(currentValue).isEqualTo(JsonPath.parse(jsonValue).json());
        } else {
            AssertionsForInterfaceTypes.assertThat(currentValue).isNotEqualTo(JsonPath.parse(jsonValue).json());
        }
    }
    ...
    void checkJsonPathIsArray(String jsonPath, int length) {
        ...
        if (length != -1) {
            AssertionsForInterfaceTypes.assertThat(((Collection) pathValue)).hasSize(length);
        }
    }
    ...
    void storeHeader(String headerName, String headerAlias) {
        ...
        AssertionsForInterfaceTypes.assertThat(headerValues).isNotEmpty();
    ...
    void checkScenarioVariable(String property, String value) {
        ...
        if (scopeValue instanceof Collection) {
            AssertionsForInterfaceTypes.assertThat(((Collection) scopeValue)).contains(value);
        } else {
